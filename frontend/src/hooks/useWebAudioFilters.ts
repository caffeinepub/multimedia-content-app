...
export function useWebAudioFilters() {
  return {
    ...
    setFilterMode: (_mode: FilterMode) => { /* no-op: filter UI removed */ },
    connectAudioElement: (_element: HTMLAudioElement) => { /* no-op: bypassed */ },
    disconnectAudioElement: () => { /* no-op: bypassed */ },
    isConnected: false,
  };
}
